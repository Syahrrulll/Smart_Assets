        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="number" name="harga_beli" id="harga_beli" value="{{ old('harga_beli') }}" placeholder="Contoh: 15000000" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 text-sm">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Umur Ekonomis</label>
                    <div class="flex items-center">
                        <input type="number" name="umur_ekonomis" id="umur_ekonomis" value="{{ old('umur_ekonomis', 5) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-l-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 text-sm border-r-0">
                        <span class="px-4 py-2.5 bg-slate-100 border border-slate-200 border-l-0 rounded-r-xl text-sm text-slate-500">Tahun</span>
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Jadwal Perawatan Berkala (Preventive)</label>
                    <div class="flex items-center">
                        <input type="number" name="maintenance_interval_months" id="maintenance_interval_months" value="{{ old('maintenance_interval_months') }}" placeholder="Kosongkan jika tidak perlu" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-l-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 text-sm border-r-0">
                        <span class="px-4 py-2.5 bg-slate-100 border border-slate-200 border-l-0 rounded-r-xl text-sm text-slate-500">Bulan</span>
                    </div>
                    <p class="text-[10px] text-slate-400 mt-1">Sistem akan mengingatkan Anda setiap X bulan.</p>
                </div>
            </div>
